...
namespace YemekSiparisUYG
{
	...
	{
		private string queryString = "SELECT COUNT(*) FROM I_urunler";
		...
		public Icecekler()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				groupBox.Controls.Add(textBox0);
					...
					string query2 = @"SELECT I_urun_image, I_urun_adi, I_urun_aciklama, I_urun_Fiyat, I_urun_adet FROM ( SELECT I_urun_image, I_urun_adi, I_urun_aciklama, I_urun_Fiyat, I_urun_adet, ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS RowNum FROM I_urunler ) AS TempTable WHERE RowNum = @Offset;";
					...
					if (reader.Read())
					{
						...
						string urunImage = reader["I_urun_image"].ToString();
						string urunAdi = reader["I_urun_adi"].ToString();
						string urunAciklama = reader["I_urun_aciklama"].ToString();
						string urunFiyat = reader["I_urun_Fiyat"].ToString();
						string urunAdet = reader["I_urun_adet"].ToString();
				...
				void Button_SepeteEkle(object sender, EventArgs e)
				{
                    ...
                    LogTut.LogKaydet("Personel", $"ID li personel İçecekler sayfasında ürün sepete ekledi: image location:{pictureBox00.ImageLocation} adı:{label0.Text} aciklama:{label00.Text} adet:{textBox0.Text}fiyat:{label000.Text}");
