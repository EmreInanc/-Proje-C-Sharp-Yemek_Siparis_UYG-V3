    ...
    public partial class YoneticiKullanicilariDuzernle : Form
    {
        ...
        private void SilBTN_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                DataYenile();
                LogTut.LogKaydet("Yönetici", $"ID li yönetici Y_yetkiler tablosundan silme işlemi yaptı , Satır Numarası{dataGridView1.SelectedRows.Count}");
            }
            else
            {
                MessageBox.Show("Lütfen silmek istediğiniz satırı seçin.");
                LogTut.LogKaydet("Yönetici", "ID li yönetici  silme işlemi yapılamadı Satır seçilemedi");
            }
            ...
            if (row != null)
            {
                ...
                using (SqlConnection connection = new SqlConnection(Connection1.ConnectionString1))
                {
                    ...
                    if (row1Yetki == "Yonetici" || row1Yetki == "Personel")
                    {
                        ...
                        LogTut.LogKaydet("Yönetici", "ID li yönetici Y_yetkiler tablosundan data değişikliği yapıldı, " +
                            "değiştirilen satır numarası:"+row.Index+"yeni bilgiler: ID:"+ row0ID +"YETKİ:"+
                             row1Yetki + "Kullanıcı Adı"+ row2KullaniciAdi+"Şifre:"+ row3Sifre);
                    }
        ...
        public void DataYenile()
        {
            ...
            LogTut.LogKaydet("Yönetici", "ID li yönetici Y_yetkiler tablosundan data yenileme işlemi yapıldı");
        }
