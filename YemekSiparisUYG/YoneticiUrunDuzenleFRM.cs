...
namespace YemekSiparisUYG
{
    public partial class YoneticiUrunDuzenleFRM : Form
    {
        ...
        private void DTVeriSil(DataGridView dt, string table, string columnName)
        {
                ...
                using (SqlConnection connection = new SqlConnection(Connection1.ConnectionString1))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            YoneticiUrunDuzenleFRM_Load(null, null);
                            LogTut.LogKaydet("Yönetici", $"ID li yönetici dönerler tablosunda satır sildi, satır numarası {dt.SelectedRows[0].Index}");
                        }
                        else
                        {
                            MessageBox.Show("Veri veritabanından silinemedi.");
                            LogTut.LogKaydet("Yönetici", $"ID li yönetici dönerler tablosunda satır sildi silerken hata oluştu {dt.SelectedRows.Count}");
                        }
                    }
                }
            }
            ...
            {
                MessageBox.Show("Lütfen silmek istediğiniz satırı seçin.");
                LogTut.LogKaydet("Yönetici", $"ID li yönetici dönerler tablosunda satır silerken hata satır silinmedi (satır sçilmedi) {dt.SelectedRows.Count}");
            }
        }
        ...
        private void YenileBTN_Click(object sender, EventArgs e)
        {
            ...
            this.Invalidate();
            this.Refresh();
            LogTut.LogKaydet("Yönetici", "ID li yönetici urunler sayfasını yeniledi" );
        }
        ...
        private void KullanicilariDuzenle_Click(object sender, EventArgs e)
        {
            ...
            if (openForm != null)
            {
                ...
                openForm.Focus();
                openForm.Show();
                LogTut.LogKaydet("Yönetici", "ID li yönetici kullanıcılar sayfasına giriş yaptı ");
            }
            else
            {
        ...
        private void GeriButton_Click(object sender, EventArgs e)
        {
            ...
            if (openForm != null)
            {
                ...
                openForm.Show();
                this.Close();
                LogTut.LogKaydet("Yönetici", "ID li yönetici giriş sayfasına geri döndü  ");
            }
            else
            {
        ...
        {
            YoneticiLogGoruntule openForm = Application.OpenForms.OfType<YoneticiLogGoruntule>().FirstOrDefault();
            if (openForm != null)
            {
                openForm.BringToFront();
                openForm.Focus();
                openForm.Show();
                this.Close();
                LogTut.LogKaydet("Yönetici", "ID Lİ Yönetici LOG SAYFASINI GÖRÜNTÜLEDİ");
            }
            else
            {
                openForm = new YoneticiLogGoruntule();
                openForm.Show();
                this.Close();
                LogTut.LogKaydet("Yönetici", "ID Lİ Yönetici LOG SAYFASINI GÖRÜNTÜLEDİ");
            }
