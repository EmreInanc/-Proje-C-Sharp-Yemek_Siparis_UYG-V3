...
{
	public partial class Iiskenderler : Form
	{
		private string queryString = "SELECT COUNT(*) FROM Ii_urunler";
		...
		public Iiskenderler()
		{
			...
			try
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					string query2 = @"SELECT li_urun_image, li_urun_adi, li_urun_aciklama, li_urun_Fiyat, li_urun_adet FROM ( SELECT li_urun_image, li_urun_adi, li_urun_aciklama, li_urun_Fiyat, li_urun_adet, ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS RowNum FROM Ii_urunler ) AS TempTable WHERE RowNum = @Offset;";
					...
					if (reader.Read())
					{
						...
						string urunImage = reader["li_urun_image"].ToString();
						string urunAdi = reader["li_urun_adi"].ToString();
						string urunAciklama = reader["li_urun_aciklama"].ToString();
						string urunFiyat = reader["li_urun_Fiyat"].ToString();
						string urunAdet = reader["li_urun_adet"].ToString();
					...
					{
						string query8 = "INSERT INTO GS_guncel_sepet (GS_urun_image,GS_urun_ad,GS_urun_aciklama,GS_urun_fiyat,GS_urun_adet) VALUES (@urunimage,@urunad,@urunaciklama,@urunfiyat,@urunadet);";
                        ...
                        LogTut.LogKaydet("Personel", $"ID li personel iskenderler sayfasından ürün sepete ekledi: image location:{pictureBox00.ImageLocation} adı:{label0.Text} aciklama:{label00.Text} adet:{textBox0.Text}fiyat:{label000.Text}");
                    }
